...
export default function useSyncedSharedData(key, initial, { lsKey, skipMarkReady = false } = {}) {
  ...
  useEffect(() => {
    ...
    loadSharedData(key).then(data => {
      ...
      if (data !== null && data !== undefined) {
        ...
        // Primer save: subir el initial al remoto.
  ...
  // Flush pendiente al desmontar (item 2.9).
  useEffect(() => () => {
    if (pendingSaveRef.current) saveSharedData(key, pendingSaveRef.current, { silent: true });
  }, [key]);

  // Wrap setState para devolver una API estable.
