      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
        {medios.map((m, i) => (
          <div key={m} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '5px 8px', background: T.faint, borderRadius: 4, border: `1px solid ${T.faint2}` }}>
            <span style={{ flex: 1, minWidth: 0, fontSize: 12, fontWeight: 600, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{m}</span>
...
function IndicesCacSection() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, alignItems: isMobile ? 'stretch' : 'flex-end', flexWrap: isMobile ? 'nowrap' : 'wrap' }}>
        <div style={isMobile ? { width: '100%' } : { width: 130 }}>
          <label style={labelSt}>Mes</label>
          <input type="month" style={inputSt} value={mes} onChange={e => setMes(e.target.value)} />
        </div>
        <div style={isMobile ? { width: '100%' } : { flex: 1, minWidth: 80 }}>
          ...
          {meses.map((mk, i) => {
            ...
            return (
              <div key={mk} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '5px 10px', borderTop: i > 0 ? `1px solid ${T.faint2}` : 'none', fontSize: 11.5 }}>
                ...
                <span style={{ fontFamily: T.fontMono, color: T.ink2, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, textAlign: 'center' }}>
      ...
      <div style={{ display: 'flex', flexDirection: isMobile ? 'column' : 'row', gap: 8, alignItems: isMobile ? 'stretch' : 'flex-end', flexWrap: isMobile ? 'nowrap' : 'wrap' }}>
        <div style={isMobile ? { width: '100%' } : { flex: 1, minWidth: 100 }}>
        ...
        <div style={isMobile ? { width: '100%' } : { width: 110 }}>
