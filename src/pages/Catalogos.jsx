    ...
    <PageLayout breadcrumb={['Catálogos', TABS[tab]]} active="Catálogos">
      ...
      <div style={{ height: isMobile ? 'auto' : 'calc(100vh - 230px)', overflow: isMobile ? 'visible' : 'hidden' }}>
        {tab === 0 && (() => {
          ...
          // recién creado aparece en el panel "Por rubro" Y en el selector al agregar.
          const rs = [...catalog.materiales.map(i => i.rubro), ...(catalog.rubros || []).map(r => r.nombre)].filter(r => r && !seen.has(r) && seen.add(r));
          return (
            <TabSimple
        ...
        {tab === 1 && (() => {
          const seen = new Set();
          // Rubros con M.O + TODOS los gremios del catálogo (para panel y selector).
          const rs = [...(catalog.subcontratos||[]).map(i => i.rubro), ...(catalog.rubros || []).map(r => r.nombre)].filter(r => r && !seen.has(r) && seen.add(r));
          return (
            <TabSimple
