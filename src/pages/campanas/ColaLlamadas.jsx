...
// Cola de llamadas REUTILIZABLE (extraída de CampLlamadas, que se retira):
// cola del día de estaciones pendientes → tarjeta grande con tel: → resultado
// en 2 taps → siguiente. P11: acá NO se muestra nada de obras ni montos.
//
// API: <ColaLlamadas onVerFicha={(operadorId) => void} filtroBandera={string|null} compacto={bool} />
// - Sin PageLayout/PageHero/guard: el que la monta pone el marco y el permiso.
// - filtroBandera: si viene, la cola trae SOLO estaciones de esa bandera (el
//   Explorador la usa para "llamadas de YPF"). Cambiarla recarga la cola.
// - onVerFicha(operadorId): el caller decide qué hacer (panel lateral,
//   navigate, etc.). Si no viene, el botón "Ver ficha" no se muestra.
// - compacto: modo panel embebido (ancho 100%, sin centrado de página, overlay
//   de éxito contenido). Sin compacto: columna centrada de 480px, como siempre.
...
// Fondos suaves a partir del color del META (hex de 6 dígitos + canal alpha).
const suave = (hex) => `${hex}1f`;
const suaveHover = (hex) => `${hex}30`;
...
// Hover con levitación suave para botones sin sombra base (se resetea al salir).
const alzar = (sombra) => (ev) => {
  ev.currentTarget.style.transform = 'translateY(-1px)';
  ev.currentTarget.style.boxShadow = sombra;
};
const posar = (ev) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      ...
      <div style={{ height: 6, borderRadius: 3, background: T.faint2, overflow: 'hidden' }}>
        <div style={{ width: `${pct}%`, height: '100%', background: T.accent, borderRadius: 3, transition: 'width 0.4s ease' }} />
      </div>
    </div>
  );
}
...
function TarjetaEstacion({ est, op, opAjeno }) {
  ...
  return (
    <div style={{
      background: 'white', border: `1.5px solid ${T.rule}`, borderRadius: 12, padding: 18,
      ...
      {telVisible ? (
        <>
          ...
          <a
            href={telHref}
            onMouseEnter={alzar('0 6px 16px -6px rgba(61,122,74,0.55)')}
            onMouseLeave={posar}
            style={{
          ...
          return (
            <button
              ...
              onMouseEnter={(ev) => {
                ...
                ev.currentTarget.style.background = suaveHover(color);
                ev.currentTarget.style.transform = 'translateY(-1px)';
                ev.currentTarget.style.boxShadow = `0 4px 12px -6px ${color}aa`;
              }}
              onMouseLeave={(ev) => {
                ev.currentTarget.style.background = suave(color);
                ev.currentTarget.style.transform = 'none';
                ev.currentTarget.style.boxShadow = 'none';
              }}
              style={{
...
function PanelDetalle({ estado, form, setForm, guardando, onGuardar, onVolver }) {
  ...
  const focoOn = (ev) => {
    ...
    ev.currentTarget.style.boxShadow = `0 0 0 3px ${T.accentSoft}`;
  };
  const focoOff = (ev) => {
    ev.currentTarget.style.borderColor = T.faint2;
    ev.currentTarget.style.boxShadow = 'none';
  };
  const inp = { style: INPUT, onFocus: focoOn, onBlur: focoOff };
  return (
    <div style={{
      ...
      {esVolver ? (
        ...
        <>
          ...
          <input {...inp} type="email" placeholder="Email del decisor" value={form.decisorEmail} onChange={set('decisorEmail')} />
          <input {...inp} placeholder="Próximo paso (ej: mandar carpeta)" value={form.proximoPaso} onChange={set('proximoPaso')} />
        </>
      )}
      <div style={{ display: 'flex', gap: 8 }}>
        <button
          onClick={onVolver}
          disabled={guardando}
          onMouseEnter={guardando ? undefined : alzar('0 4px 10px -6px rgba(20,18,15,0.4)')}
          onMouseLeave={posar}
          style={{ ...BTN_BASE, flex: 1, background: T.faint, color: T.ink2 }}
        >
          ← Volver
        </button>
        <button
          onClick={onGuardar}
          disabled={guardando}
          onMouseEnter={guardando ? undefined : alzar('0 6px 16px -6px rgba(26,155,156,0.55)')}
          onMouseLeave={posar}
          style={{ ...BTN_BASE, flex: 2, background: T.accent, color: 'white', opacity: guardando ? 0.6 : 1 }}
        >
...
function AvisoColision({ nombre, canal, onSaltear }) {
  return (
    <div style={{
      background: '#fdf3e3', border: `1.5px solid ${T.warn}`, borderRadius: 12, padding: 14,
      display: 'flex', flexDirection: 'column', gap: 10, animation: 'colaLlamadasIn 0.25s ease',
    }}>
      ...
      <button
        onClick={onSaltear}
        onMouseEnter={alzar('0 6px 16px -6px rgba(212,146,58,0.55)')}
        onMouseLeave={posar}
        style={{ ...BTN_BASE, background: T.warn, color: 'white' }}
      >
        Saltear →
      </button>
    </div>
...
function PantallaCentrada({ icono, titulo, sub, compacto, children }) {
  return (
    <div style={{
      maxWidth: compacto ? '100%' : 480, margin: compacto ? 0 : '0 auto',
      padding: compacto ? '28px 12px' : '48px 16px',
...
export default function ColaLlamadas({ onVerFicha, filtroBandera = null, compacto = false }) {
  ...
  // El guard de permisos vive en quien monta el componente (página/Explorador).
  const usuarioId = currentUser?.id;
  const listo = !!currentUser;
  ...
  // Bandera a la que corresponde la cola CARGADA (se setea solo en .then/.catch).
  // Mientras difiera del filtro pedido, mostramos "cargando" (derivado, sin
  // setState en effect) — cubre el cambio de filtroBandera con cola vieja en pantalla.
  const [banderaCola, setBanderaCola] = useState();
  ...
  // respuestas viejas si el filtro cambió con un fetch en vuelo.
  const filtroRef = useRef(filtroBandera ?? null);
  // Última bandera efectivamente cargada (para resetear vistas/salteadas al cambiar).
  const banderaCargadaRef = useRef();
  // Self-ref para relanzar la carga desde .finally si el filtro cambió en vuelo.
  const cargarColaRef = useRef(null);
  ...
  //    estado inicial, de los handlers, de `agotada` o de banderaCola ≠ filtro.
  const cargandoRef = useRef(false);
  const cargarCola = useCallback(() => {
    if (cargandoRef.current) return;
    cargandoRef.current = true;
    const bandera = filtroRef.current;
    const cambioFiltro = banderaCargadaRef.current !== bandera;
    // Filtro nuevo → las vistas/salteadas de la bandera anterior no aplican.
    if (cambioFiltro) vistasRef.current = new Set();

    const trabajo = async () => {
      const res = await Promise.all(ESTADOS_COLA.map((estadoLlamada) =>
        fetchEstaciones({
          filtros: { estadoLlamada, ...(bandera ? { bandera } : {}) },
          page: 1, pageSize: PAGE_COLA, orden: 'updated_at',
        })));
    ...
    trabajo()
      .then(({ ops, lista, quedan: q }) => {
        // Respuesta de un filtro que ya no es el pedido → descartar (el
        // .finally relanza la carga con el filtro vigente).
        if (filtroRef.current !== bandera) return;
        banderaCargadaRef.current = bandera;
        setBanderaCola(bandera);
        if (cambioFiltro) setSalteadasN(0);
        setOperadores(ops);
        setCola(lista);
        setIdx(0);
        setQuedan(q);
      ...
      .finally(() => {
        ...
        // El filtro cambió mientras cargábamos → relanzar con el vigente.
        if (filtroRef.current !== bandera) cargarColaRef.current?.();
      });
  }, [fetchEstaciones]);

  useEffect(() => { cargarColaRef.current = cargarCola; }, [cargarCola]);

  // Carga inicial + recarga al cambiar el filtro de bandera.
  useEffect(() => {
    filtroRef.current = filtroBandera ?? null;
    if (listo) cargarCola();
  }, [listo, filtroBandera, cargarCola]);
  ...
  // "Ver ficha" delega en el caller (panel lateral del Explorador, navigate, etc.).
  const verFicha = useCallback(() => {
    const est = cola[idx];
    if (est && typeof onVerFicha === 'function') onVerFicha(est.operador_id ?? null);
  }, [cola, idx, onVerFicha]);
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  // La cola en memoria solo vale si corresponde al filtro pedido (si el filtro
  // cambió, el effect ya disparó la recarga → mientras tanto, "cargando").
  const colaVigente = banderaCola === (filtroBandera ?? null);
  const est = faseCola === 'lista' && colaVigente ? cola[idx] : null;
  ...
  const hayVerFicha = typeof onVerFicha === 'function';

  let cuerpo;
  if (!listo || faseCola === 'cargando' || agotada || !colaVigente) {
    cuerpo = <PantallaCentrada compacto={compacto} icono="📞" titulo="Cargando la cola del día…" />;
  } else if (faseCola === 'error') {
    cuerpo = (
      <PantallaCentrada compacto={compacto} icono="🤦" titulo="Algo salió mal cargando la cola" sub={errorCola}>
    ...
    cuerpo = (
      <PantallaCentrada
        ...
        titulo={hechasHoy > 0
          ...
          : `No hay estaciones pendientes de llamar${filtroBandera ? ` de ${filtroBandera}` : ''}`}
        sub={salteadasN > 0
    ...
    cuerpo = (
      <div style={{
        maxWidth: compacto ? '100%' : 480, margin: compacto ? 0 : '0 auto',
        display: 'flex', flexDirection: 'column', gap: 14, paddingBottom: compacto ? 8 : 24,
      }}>
        <BarraProgreso hechasHoy={hechasHoy} quedan={quedan} />
        {/* key={est.id}: cada estación entra con su transición (fade + subida suave). */}
        <div key={est.id} style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'colaLlamadasIn 0.3s ease' }}>
          <TarjetaEstacion est={est} op={op} opAjeno={opAjeno} />
          <div style={{ display: 'flex', gap: 8 }}>
            <Btn sm style={{ flex: 1 }} onClick={saltear}>Saltear →</Btn>
            {hayVerFicha && <Btn sm style={{ flex: 1 }} onClick={verFicha}>Ver ficha</Btn>}
          </div>
          {colision ? (
  ...
  return (
    <div style={{ position: 'relative' }}>
      ...
      {exito && (
        <div style={{
          // En modo página tapa toda la pantalla (como siempre); embebido queda
          // contenido en el panel para no tapar el resto del Explorador.
          position: compacto ? 'absolute' : 'fixed', inset: 0, zIndex: compacto ? 20 : 2000,
          display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
          gap: 8, background: 'rgba(247,244,236,0.88)', borderRadius: compacto ? 12 : 0,
        }}>
      ...
      <style>{`
        @keyframes colaLlamadasPop { 0% { transform: scale(0.3); opacity: 0; } 100% { transform: scale(1); opacity: 1; } }
        @keyframes colaLlamadasIn { 0% { transform: translateY(10px); opacity: 0; } 100% { transform: none; opacity: 1; } }
      `}</style>
    </div>
  );
}
