-- ============================================================================
-- Seed de datos TRUCHOS del módulo Campañas para Kamak-Pruebas.
-- ⚠️ SOLO PRUEBAS: se aplica vía .github/workflows/db-pruebas.yml (ref de
-- pruebas hardcodeado). Idempotente: ids fijos op_seed_*/est_seed_*/... con
-- ON CONFLICT DO NOTHING — se puede re-correr sin duplicar.
...
-- Anti-colisión de ejemplo: 3 operadores "en tratativas" con Franco
update camp_operadores set
  en_tratativas = true,
  owner_user_id = (select id from app_users where lower(email) = 'fgeespinoza@gmail.com' limit 1),
...
select
  ...
  'Estación Trucha ' || (array['Norte','Sur','Centro','Ruta'])[1 + i % 4] || ' ' || lpad(i::text, 3, '0'),
  'Av. Siempreviva Falsa ' || (100 + i * 7),
  (array['Necochea','Quilmes','Bahía Blanca','Córdoba','Rosario','Mendoza','San Justo','Tandil'])[1 + i % 8],
  (array['Buenos Aires','Buenos Aires','Buenos Aires','Córdoba','Santa Fe','Mendoza','Buenos Aires','Buenos Aires'])[1 + i % 8],
...
select
  ...
  'op_seed_' || lpad(i::text, 3, '0'),
  (array['Juan','María','Carlos','Lucía','Pedro','Sofía','Diego','Ana'])[1 + i % 8] || ' ' ||
  (array['Trucho','Fictini','Demozzi','Pruebalez','Ejemplar'])[1 + i % 5],
  (array['Dueño','Gerente de Operaciones','Resp. Expansión','Jefa de Tiendas','Socio Gerente'])[1 + i % 5],
...
insert into camp_listas (id, nombre, canal, tipo, descripcion, costo_mensual, activa) values
  ...
  ('lst_seed_004', 'Meta CTWA Julio (trucha)',    'whatsapp', 'ads',       'Campaña de anuncios de prueba',      150,   true)
on conflict (id) do nothing;
...
-- ~300 actividades de las últimas 8 semanas, variadas por canal/tipo
...
select
  ...
  case when 1 + i % 10 <= 3 then (array['SIN LLAMAR','NO ATIENDE','VOLVER A LLAMAR','PASÓ MAIL','PASÓ WHATSAPP','DECISOR IDENTIFICADO','NO INTERESA','LEAD CALIENTE'])[1 + i % 8] end,
  'Actividad de prueba #' || i,
  case when 1 + i % 10 <= 3
    then coalesce((select id from app_users where lower(email) = 'admkamakdesarrollos@gmail.com' limit 1), 'seed')
    else coalesce((select id from app_users where lower(email) = 'fgeespinoza@gmail.com' limit 1), 'seed')
  end,
...
-- Administración = el rol de Carolina en la vida real).
update app_users
  set permisos = coalesce(permisos, '{}'::jsonb) || '{"campanas": true}'::jsonb
  where rol = 'Administración' and (permisos ->> 'campanas') is distinct from 'true';
