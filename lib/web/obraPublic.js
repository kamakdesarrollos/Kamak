...
export function obrasPublicadas(blob) {
  ...
  return obras
    // Doble gate: publicada Y finalizada. El estado se chequea acá (fuente de
    // verdad) para que reabrir una obra (finalizada→activa) la saque de la web
    // automáticamente, sin depender del botón del editor.
    .filter(o => o.web && o.web.publicar === true && o.estado === 'finalizada')
    .map(obraPublic)
