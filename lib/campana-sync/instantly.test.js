...
it('status mapeado a texto; códigos desconocidos quedan como número', async () => {
  ...
  const lista = {
    items: [
      ...
      // 99: desconocido POSITIVO queda como número (los negativos ahora se
      // filtran por ser campañas ocultas/borradas de Instantly — test aparte).
...
it('campañas con status negativo (ocultas/borradas por Instantly, ej. la [AI SDR] de muestra) se filtran', async () => {
  vi.stubEnv('INSTANTLY_API_KEY', 'k');
  const lista = {
    items: [
      ...LISTA.items,
      {
        id: '0197ffff-dead-7ab1-b1c2-00000000dead',
        name: '[AI SDR] Kamak AI Sales Agent - Fully Personalized Campaign',
        status: -1, // Instantly la esconde en su UI pero la reporta por API
        timestamp_created: '2026-07-13T20:21:29.769Z',
      },
    ],
  };
  vi.stubGlobal('fetch', mkFetch({ lista }));
  const r = await sync();
  expect(r.ok).toBe(true);
  expect(r.filas.map((f) => f.campana_ext_nombre)).not.toContain(
    '[AI SDR] Kamak AI Sales Agent - Fully Personalized Campaign'
  );
  expect(r.filas).toHaveLength(2);   // solo las 2 visibles
  expect(r.campanas).toBe(2);        // el count tampoco cuenta la oculta
});
