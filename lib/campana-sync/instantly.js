...
export async function sync() {
  ...
  for (const c of lista) {
    // status negativo (ej. -1) = campaña que Instantly BORRÓ/OCULTÓ de su
    // propia interfaz pero sigue reportando por API (caso real: la "[AI SDR]"
    // de muestra que Instantly auto-crea). No es una campaña del usuario → afuera.
    if (typeof c?.status === 'number' && c.status < 0) { ocultas += 1; continue; }
  ...
  return { ok: true, filas, campanas: lista.length - ocultas, conAnalytics };
}
